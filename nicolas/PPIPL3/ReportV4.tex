...
(PPI) suggest that the higher the number of paths of length 3 (L3)
between two proteins, the more likely they are to interact. This paper
extends previous work based on the L3 principle by taking into account
the representation learning of node features of the PPI network. In
particular, we train an \texttt{XGBoost} model using L3 and handcrafted
...
yet unknown interactions. The focus of this study is to evaluate different
methods for predicting PPIs using the existing knowledge of the network
of interactions, which is represented as an undirected graph. The
traditional way of approaching the problem is usually based on social
networks analysis, more specifically on the triadic closure principle
(TCP), that states that the more common shared friends that two people
have, the more likely that they know each other. As shown by previous
studies, the mentioned approach fails because it does not consider
the structural and chemical properties of the proteins \cite{Kovacs2019}.
...
\item [{Length-3~Path~Counts~(A3)}] This is the simplest implementation
of the proposed insight of ``if my friends and your friends interact,
...
\par\end{centering}
\begin{centering}
\includegraphics[width=0.7\columnwidth]{ML_Metric/Imp\lyxdot A2\lyxdot All}
\par\end{centering}
\begin{centering}
\includegraphics[width=0.7\columnwidth]{ML_Metric/Imp\lyxdot A3\lyxdot All}
\par\end{centering}
\centering{}\includegraphics[width=0.7\columnwidth]{ML_Metric/Imp\lyxdot L3\lyxdot All}
\end{figure}
...
are presented in Figures \ref{HI1}, \ref{HI2} and \ref{HI3}. 

\begin{figure}[H]
\noindent \begin{centering}
\caption{\label{HI1}\emph{HI-II-14} results for \texttt{Node2Vec} with CN
feature}
\par\end{centering}
...
It can be seen that for \emph{HI-II-14}, CN performs marginally better
than L3 when comparing precision ($0.9978$ vs $0.9977$) but worse
in terms of recall ($0.9346$ vs $0.9598$), although all metrics
have a good values for area under the ROC curve of 0.99.
...
\ref{HI1-1}, \ref{HI2-1} and \ref{HI3-1}. It can be seen that in
terms of false positives, CN performs better than A3 and L3, but in
terms of false negatives, A3 actually performs better that CN and
...
An interesting point from this human datasets can be observed when
looking at the importance plots, that show a very biased influence
of the handcrafted features. Figure X shows this behavior for the
\emph{HI-II-14} dataset, which is very similar to \emph{HI-TESTED}.

\begin{figure}[H]
\noindent \begin{centering}
\caption{\label{F8-importance-1}Importance gain plots for \emph{HI-II-14}}
\par\end{centering}
\begin{centering}
\includegraphics[width=0.7\columnwidth]{Human/Imp\lyxdot A2\lyxdot 1}
\par\end{centering}
\begin{centering}
\includegraphics[width=0.7\columnwidth]{Human/Imp\lyxdot A3\lyxdot 1}
\par\end{centering}
\centering{}\includegraphics[width=0.7\columnwidth]{Human/Imp\lyxdot L3\lyxdot 1}
\end{figure}
...
It was found that applying \texttt{XGBoost} to the rice PPI prediction
obtained very good results when only using the node embedding technique
(AUC=0.93) and when only using the handcrafted features based on protein
neighborhood (AUC \textasciitilde 0.88). Even better results were
achieved when the handcrafted features and the vector representation
information were combined (AUC \textasciitilde 0.98). It is important
to consider that the importance gain of the handcrafted feature in
each model was high, although importance values for the A3 method
stand out less from the node embedding information than in CN and
L3.

On the human interactome, the results show that on both interactomes
(\emph{HI-II-14} and \emph{HI-TESTED}) all metrics yield very good
results for the link prediction problem, and therefore this study
extends previous work by other authors that only consider the predictive
power of the metrics alone.

It is important to note the difference in feature importance from
rice and human interactomes. In the former, features have a more balanced
importance distribution than in the latter dataset, whose model shows
a very strong dependence on the handcrafted feature.
...
on rice and human interactomes, but can be easily extended to other
organisms or even to other graphs not necessarily related to biology. 
