...
Recent developments in network-based prediction of unmapped protein-protein
interactions (PPIs) reveal that two proteins are more likely to interact,
the higher the number of paths of length 3 between them, normalized
by the geometric average of their interactions. This paper extends
previous work on predicting binary interactions by taking into account
the learning of features (embeddings) of the PPI network. In particular,
we implement a gradient boosted decision tree model (\texttt{XGBoost})
using handcrafted features (including the normalized measure of paths
of length 3, L3) and embeddings from an algorithm that generates a
low-dimensional representation of nodes (\texttt{Node2Vec}). Our main
result shows that while L3 remains an important feature for predicting
PPIs, a better performance is achieved when L3 is combined with embedding
features. The proposed approach is validated for the human and rice
interactomes. For both cases, higher AUC values are obtained when
both types of features are used to train the model. The developed
framework can also be applied to interactomes of other organisms for
which a PPI network become recently available.
\end{abstract}
...
reporter gene that activates by the binding of a DNA-binding Domain
(DB) and an Activation Domain (AD) of a Transcription Factor that
binds to an Upstream Activation Sequence (UAS). To evaluate an interaction,
the Y2H approach fuses a protein to the DB domain (known as \emph{bait})
and another protein to the AD (known as \emph{prey}). If the proteins
interact, then the reporter gene expression is activated by the AD
(Fig. \ref{Y2H}B). Otherwise, if proteins fail to interact, then
the reporter gene is not expressed (Fig. \ref{Y2H}C).
...
interactome (\emph{HI-II-14}, \emph{HI-TESTED})\cite{Rolland2014Human}
