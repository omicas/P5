...
\maketitle
\begin{abstract}
Networks science provides a set of computational tools to identify unmapped protein-protein interactions (PPI's). Barab\'asi et al. recently introduced a metric (based on network paths of length three) which 
proved to be an important feature in predicting links of the interactome network. In other words, paths of length three (L3) enable us to discover hidden link candidates for experimental validation. This paper extends the previous approach by Barab\'asi et al. using \texttt{XGBoost} module of Machine Learning (ML). We train the model using handcrafted features as well as features learned from embeddings. For the feature extraction from the network, \texttt{Node2Vec} is applied. Results show that while L3 feature remains important for predicting links, so are the features retrieved from the network structure using \texttt{Node2Vec}. The mentioned approaches are compared using the human and the rice interactomes.
\end{abstract}
...
\bibliography{\string"refs\string"}
