...
\usepackage{babel}
\begin{document}
\title{Enhanced Prediction of Protein-Protein Interactions on the Human and
Rice Interactome with Machine Learning}
\author{Nicolás Antonio López Rozo}
\maketitle
\begin{abstract}
Computational tools offer an alternative to identify unmapped, biologically
significant protein-protein interactions (PPI's). A novel approach
by Barabási et al, based on network paths of length three (L3) on
the interactome, outperforms all existing link prediction methods.
L3 can offer therefore a new approach to reveal a smaller subset of
potential interactions for a more affordable experimental validation. 

In this study, the previous approach for link prediction is expanded
by using \texttt{XGBoost} module of Machine Learning (ML). For the
feature extraction from the network, the \texttt{Node2Vec} module
was applied, although L3 information is added as a feature for improving
link prediction. Results show that L3 feature is very important for
the ML procedure, although a significant feature importance lies on
the features retrieved from the network structure using \texttt{Node2Vec}.
The mentioned approaches are compared using the human and the rice
interactomes.
\end{abstract}
...
(CN, A3, L3). For this report, the human interactome was used (\emph{HI-II-14.txt}).
A curated version of the mentioned network (\emph{HI-TESTED.txt})
...
the dataset \emph{HI-II-14.txt} and \emph{HI-TESTED.txt} are used
for prediction and the dataset \emph{HI-III.txt} is used for validation.
...
\noindent \centering{}\includegraphics[width=0.8\columnwidth]{\string"/home/ocin/Documents/02 Asignaturas/10 GraphAnalytics/Project1/TCP_extract\string".eps}
\end{figure}
...
\begin{figure}[h]
\caption{Methods Comparison for \emph{HI-II-14.txt}}
...
\begin{figure}[h]
\caption{Methods Comparison for \emph{HI-TESTED.txt}}
...
and when classical length-2 paths are used, the predicted protein
...
\bibliography{\string"/home/ocin/Documents/02 Asignaturas/refs\string"}
